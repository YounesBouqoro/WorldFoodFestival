...
function render(data){
  const order=data.order||{};const business=data.business||{};const items=data.items||[];
  ...
  paper.innerHTML=`
    ...
    <section class="receipt-lines">
      ${items.map(item=>`<div class="receipt-line"><div><strong>${item.quantity}× ${escapeHtml(item.name)}</strong><small>${euro(item.unit_price)} / Stück${Number(item.discount_amount||0)>0?` · Rabatt −${euro(item.discount_amount)}`:''}</small>${Number(item.deposit_qty||0)>0?`<small>${item.deposit_qty}× Pfand ${euro(item.deposit_unit_price)}</small>`:''}</div><div class="receipt-line-price">${euro(item.line_total)}</div></div>`).join('')}
    </section>
    <section class="receipt-totals">
      <div class="receipt-total-row"><span>Produkte</span><strong>${euro(order.product_subtotal)}</strong></div>
      <div class="receipt-total-row"><span>Pfand</span><strong>${euro(order.deposit_total)}</strong></div>
      ${Number(order.discount_amount||0)>0?`<div class="receipt-total-row"><span>Studentenrabatt</span><strong>−${euro(order.discount_amount)}</strong></div>`:''}
      <div class="receipt-total-row grand"><span>Gesamt</span><strong>${euro(order.total_amount)}</strong></div>
    </section>
    <section class="tax-box">
      <h2>Steuerübersicht</h2>
      ${taxes.map(([rate,part])=>`<div class="tax-row"><span>${rate.toLocaleString('de-DE')} % MwSt. aus ${euro(part.gross)}</span><strong>${euro(part.tax)}</strong></div>`).join('')||'<div class="tax-row"><span>Keine Steuerdaten</span><strong>–</strong></div>'}
      <div class="tax-row"><span>Netto gesamt</span><strong>${euro(order.net_total)}</strong></div>
      <div class="tax-row"><span>Steuer gesamt</span><strong>${euro(order.tax_total)}</strong></div>
    </section>
...
async function load(){
  ...
  const {data,error}=await db.rpc('get_receipt',{p_token:token});
  if(error||!data){paper.innerHTML='<div class="receipt-error">Beleg konnte nicht geladen werden.</div>';return}
  document.title=`Beleg #${data.order?.order_no||''} – World Food Festival`;render(data);
}
